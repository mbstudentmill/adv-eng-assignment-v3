...
-- Create the dataset
CREATE SCHEMA IF NOT EXISTS `ade-adveng-assign.imdb_warehouse`
OPTIONS(
...
ON `ade-adveng-assign.imdb_warehouse.dim_title`(tconst);

CREATE OR REPLACE INDEX idx_dim_person_nconst 
ON `ade-adveng-assign.imdb_warehouse.dim_person`(nconst);

CREATE OR REPLACE INDEX idx_fact_rating_tconst 
ON `ade-adveng-assign.imdb_warehouse.fact_title_rating`(tconst);

CREATE OR REPLACE INDEX idx_fact_rating_genre 
ON `ade-adveng-assign.imdb_warehouse.fact_title_rating`(genre);
...
-- GRANT `roles/bigquery.dataViewer` ON SCHEMA `ade-adveng-assign.imdb_warehouse` TO "user:your-email@domain.com";
-- GRANT `roles/bigquery.dataEditor` ON SCHEMA `ade-adveng-assign.imdb_warehouse` TO "user:your-email@domain.com";
