...
-- BigQuery Data Warehouse DDL for IMDb Star Schema
-- This creates a data warehouse with partitioning and clustering for the assignment

-- Create the dataset (replace with your project ID)
CREATE SCHEMA IF NOT EXISTS `YOUR_PROJECT_ID.imdb_warehouse`
OPTIONS(
  description = 'IMDb Data Warehouse for Advanced Data Engineering Assignment',
  labels = [('project', 'ade-assignment'), ('environment', 'development')]
);

-- 1. DIMENSION TABLE: dim_title (Title Information)
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.dim_title` (
...
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.dim_person` (
...
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.dim_genre` (
...
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.dim_region` (
...
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.dim_date` (
...
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.bridge_title_genre` (
  tconst STRING NOT NULL,
  genre_id STRING NOT NULL,
  created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP()
)
...
CREATE OR REPLACE INDEX idx_dim_title_tconst 
ON `YOUR_PROJECT_ID.imdb_warehouse.dim_title`(tconst);

CREATE OR REPLACE INDEX idx_dim_person_nconst 
ON `YOUR_PROJECT_ID.imdb_warehouse.dim_person`(nconst);

CREATE OR REPLACE INDEX idx_fact_rating_tconst 
ON `YOUR_PROJECT_ID.imdb_warehouse.fact_title_rating`(tconst);

CREATE OR REPLACE INDEX idx_fact_rating_genre 
ON `YOUR_PROJECT_ID.imdb_warehouse.fact_title_rating`(genre);

-- Insert sample data into dimension tables
INSERT INTO `YOUR_PROJECT_ID.imdb_warehouse.dim_genre` (genre_id, genre_name, genre_description)
VALUES
...
CREATE OR REPLACE VIEW `YOUR_PROJECT_ID.imdb_warehouse.v_top_rated_movies` AS
SELECT 
  t.primary_title,
  t.start_year,
  t.genres,
  f.average_rating,
  f.num_votes
FROM `YOUR_PROJECT_ID.imdb_warehouse.fact_title_rating` f
JOIN `YOUR_PROJECT_ID.imdb_warehouse.dim_title` t ON f.tconst = t.tconst
...
CREATE OR REPLACE VIEW `YOUR_PROJECT_ID.imdb_warehouse.v_genre_performance` AS
SELECT 
  g.genre_name,
...
FROM `YOUR_PROJECT_ID.imdb_warehouse.fact_title_rating` f
JOIN `YOUR_PROJECT_ID.imdb_warehouse.dim_genre` g ON f.genre = g.genre_id
WHERE f.num_votes >= 100
GROUP BY g.genre_name
ORDER BY avg_rating DESC;

-- Grant permissions (adjust as needed for your GCP setup)
-- GRANT `roles/bigquery.dataViewer` ON SCHEMA `YOUR_PROJECT_ID.imdb_warehouse` TO "user:your-email@domain.com";
-- GRANT `roles/bigquery.dataEditor` ON SCHEMA `YOUR_PROJECT_ID.imdb_warehouse` TO "user:your-email@domain.com";

-- Create table for monitoring and audit
CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.imdb_warehouse.etl_audit_log` (
